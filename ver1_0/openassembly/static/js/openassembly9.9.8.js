...
function refresh_dashboard(path, dash_id){
  ...
  $.post("/add_board/", {path: path, dashboard_id: dash_id, type: 'refresh'},
  function(data) {
      if(data.FAIL === true){
         //$(ui.item).fadeOut('slow', function() {
          //    $(ui.sender).append(ui.item);
      ...
      if(data.FAIL === false){
          ...
          if(data.rendertype == 'chat'){
            ...
            $('#panel_container' + data.dashpk).html(data.html);
          }
      }
  }, "json");
}

function push_dashboard(path, dash_id){
  
  $.post("/add_board/", {path: path, dashboard_id: dash_id, type: 'add'},
  function(data) {
      if(data.FAIL === true){
         //$(ui.item).fadeOut('slow', function() {
          //    $(ui.sender).append(ui.item);
